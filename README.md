...
> 📁 **test** 폴더에는 AI Agent 서비스 평가·검증을 위하여 1차 가공한 검증용 데이터셋이 있습니다. **data** 폴더에서는 가공 전 원본 데이터셋을 확인 가능합니다. (대용량 파일은 source 링크를 들어가서 직접 다운로드 받아야 합니다.)
...
## 5. Test
> 📁 **test** 데이터는 AI Agent 서비스 평가·검증을 위한 Q/A 데이터셋이며, 원본 **data**에서 1차 가공한 결과입니다. 가공 과정에서는 (예: 필요한 필드만 추출 / 포맷을 jsonl로 통일 / 불필요 메타 제거 등) 을 수행했습니다.
